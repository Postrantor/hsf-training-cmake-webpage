...
system. `-t` (`--target` before CMake 3.15) lets you select a target. There's also a `cmake <dir> --install` command in CMake 3.15+ that does the install - without invoking the underlying build system!
...
> at an _existing_ build directory from anywhere.
> {: .discussion}
